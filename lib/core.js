...
(Server bug: Having Noctenium Infusion in inventory causes skill delays)`)
		...
		for(let i = 0, chain = info.userChain; i < 10; i++, chain = undefined) { // Limit recursion to prevent infinite loop
			...
			if(chain == null && clientAction) {
				...
				if(categoryChains) {
					const category = get(skillsClient, (clientAction.templateId === 11013 ? 10813 : clientAction.templateId), clientAction.skill.id, 'category')
			...
			if(chain != null) {
				...
				if(chainSkill.equals(skill) || !get(skillsClient, (player.templateId === 11013 ? 10813 : player.templateId), chainSkill.id)) break
		...
		const currentSkillCategory = get(skillsClient, (player.templateId === 11013 ? 10813 : player.templateId), skill.id, 'category')
	...
	function skillInfo(skill) {
		...
		let info = [ // Ordered by least specific < most specific
			get(skillsClient, (player.templateId === 11013 ? 10813 : player.templateId), skill.id),
			get(skills, '*', skill.id),
			get(skills, job, '*'),
