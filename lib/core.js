...
module.exports = function SPCore(mod) {
	const skillsClient = require('../config/skills_client' + (mod.majorPatchVersion < 82 ? '_81' : ''))
	const skills = require('../config/skills'+ (mod.majorPatchVersion < 82 ? '_81' : ''))
	...
	function startSkill(type, version, code, data) {
		...
		const event = mod.dispatch.fromRaw(type, version, data = Buffer.from(data)),
